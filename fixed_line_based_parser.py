...
class FixedLineBasedHochmaParser:
    def __init__(self, db_path="bible_database.db"):
        """
        수정된 줄바꿈 기반 호크마 성경주석 파서 (<br> 태그 처리)
        - 19:11 (단일 절)
    ...
    def extract_fixed_line_based_commentary(self, soup, url):
        """수정된 줄바꿈 기반 주석 데이터 추출"""
        article_data = {
            'url': url,
            'title': '',
            'commentary_name': '',
        ...
        # 3. 본문에서 <br> 태그를 고려한 줄바꿈 기반 절별 주석 추출
        ...
        if content_element:
            ...
            if verse_separators:
                ...
                for j, (line_idx, separator_text, verses) in enumerate(verse_separators):
                    ...
                    print(f"  절 구분자 '{separator_text}' -> {len(verses)}개 절, {len(verse_content)}자")
            
            # 절 구분자가 없는 경우 전체 텍스트를 하나의 주석으로 처리
            elif len(lines) > 0:
                full_content = '\n'.join(lines)
                if len(full_content) > 100 and article_data['chapter']:
                    article_data['verse_commentaries'].append({
    ...
    def parse_to_excel(self, article_id, excel_filename=None):
        ...
        article_data = self.extract_fixed_line_based_commentary(soup, url)
...
def main():
    ...
    print("수정된 줄바꿈 기반 호크마 성경주석 파서 (<br> 태그 처리)")
