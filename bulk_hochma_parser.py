...
class BulkHochmaParser:
    ...
    def save_to_excel(self, filename=None):
        ...
        if not filename:
            ...
            filename = f"hochma_all_commentaries_{timestamp}.xlsx"
...
def main():
    """메인 함수"""
    parser = BulkHochmaParser()
    
    # 1. 게시글 ID 수집
    print("1️⃣ 게시글 ID 수집...")
    
    # 기존 JSON 파일이 있는지 확인
    json_files = [f for f in os.listdir('.') if f.startswith('hochma_article_ids_') and f.endswith('.json')]
    
    if json_files:
        # 가장 최근 파일 사용
        latest_file = sorted(json_files)[-1]
        print(f"   기존 ID 파일 사용: {latest_file}")
        ...
        # 새로 수집
        from find_all_articles import find_all_article_ids
